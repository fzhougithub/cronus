-- DROP PROCEDURE stg_global.initial_dur_history(text, text, text, text);

CREATE OR REPLACE PROCEDURE stg_global.initial_dur_history(IN input_schema text, IN input_table text, IN input_customer text, IN input_set text)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    each_thread RECORD;
...
BEGIN
	...
	LOOP
		...
		*/
		target_table := 'dur_hist_1unlog'||v_customer::text||'_'||input_set::text;
		...
		threads_table:= 'dur_threads_'||v_customer::text;
	...
	--RAISE NOTICE '% Loaded % records of customer_id % to %.%', CURRENT_TIMESTAMP, v_count, input_customer, target_schema,target_table;
